...
#	parameters enterd.
# 01/10/17: added the "javaoption" to increse the java heap.
#------------------------------------------------------------------
if [[ $# -lt 2 ]];then
	if [[ $# -lt 1 ]]; then
		...
		echo -e "\tplease use the MM_Deploy_updt_WAR.sh script. \n"
		exit 1
	else
		## TODO:
		...
		cd ${WAR_BASE}
        	ls -C1 Mappin*.war
        	echo -e "\nYou may copy and paste you choice on the line below - "
       	 	echo -e " \tpaste here - \c"
        	read UPDT_warfile
...
echo -e "\nJYTN0101I: standby loading script ... "
# try to input the war file as a parameter to the script
./wsadmin.sh -conntype SOAP -javaoption "-Xms512m -Xmx16512m" -username wsusrp -password 3106WAS! -f ${Userbase}/${1}
#
echo " ${0} done"
exit 0
